...
function mergeProgress(server, client, { preferClientPrefs = true } = {}) {
  ...
  return {
    ...
    settings: {
      ...
      displayScale: pickNum(pref.settings?.displayScale, other.settings?.displayScale),
      refreshRate: pickNum(pref.settings?.refreshRate, other.settings?.refreshRate),
      debugLayer: !!(pref.settings?.debugLayer || other.settings?.debugLayer),
    },
    displayScale: pickNum(pref.displayScale, other.displayScale) ?? 1,
    refreshRate: pickNum(pref.refreshRate, other.refreshRate) ?? 60,
    debugLayer: !!(pref.debugLayer || other.debugLayer),
  };
}
