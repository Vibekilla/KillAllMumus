 ...
 * Spec: https://bobina.moe/.well-known/openid-configuration
...
/**
 * Mount Bobina auth routes on an Express app.
...
function mountBobinaAuth(app, pool) {
  ...
  app.get('/auth/bobina', async (req, res) => {
    try {
      ...
      await pool.query(
        `INSERT INTO oauth_states (state, code_verifier, nonce) VALUES ($1, $2, $3)`,
  ...
  app.get('/auth/bobina/callback', async (req, res) => {
    try {
      ...
      const st = await pool.query(
        `DELETE FROM oauth_states WHERE state = $1 RETURNING code_verifier, nonce, created_at`,
        [String(state)]
      );
      if (!st.rows.length) return res.status(400).send('Invalid or expired state');
      const { code_verifier: verifier, nonce } = st.rows[0];
      ...
      await pool.query(
        ...
        [
          bcId,
          claims.username || claims.preferred_username || null,
          claims.picture || null,
          claims.x_id != null ? String(claims.x_id) : null,
          claims.x_username || null,
        ]
      );
  ...
  app.get('/api/me', async (req, res) => {
    try {
      ...
      if (!player) return res.json({ authenticated: false, bobinaConfigured: configured() });
      res.json({
        authenticated: true,
        bobinaConfigured: configured(),
        bcId: player.bc_id,
        username: player.cached_username,
        avatar: player.cached_avatar,
        xId: player.cached_x_id,
        xUsername: player.cached_x_username,
        profileUrl: player.cached_username
          ? `https://bobina.moe/${player.cached_username}`
          : null,
      });
    } catch (e) {
...
<p>Register <strong>killallmumus.com</strong> as an OAuth app on bobina.moe
(System → OAuth Apps) with scopes <code>openid profile social</code> and redirect URI
<code>https://killallmumus.com/auth/bobina/callback</code>.</p>
<p>Then set <code>BOBINA_CLIENT_ID</code> and <code>BOBINA_CLIENT_SECRET</code> in the server <code>.env</code> and restart.</p>
