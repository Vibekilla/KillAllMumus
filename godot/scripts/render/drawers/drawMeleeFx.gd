...
func _draw_slash(f: Dictionary, pr: float) -> void:
	...
	# Narrow arcs (lash) need a thicker stroke or they vanish on dark stage
	var thick: float = 6.0 + charge * 9.0
	if key == "lash":
		thick = 9.0 + charge * 10.0
	...
	ctx.line_join("round")
	# Outer glow crescent
	ctx.global_alpha(al * 0.85)
	ctx.stroke_style(col)
	ctx.line_width(thick)
	ctx.shadow_color(col)
	ctx.shadow_blur(18)
	ctx.begin_path()
	ctx.arc(0, 0, rad, a0, a1)
	ctx.stroke()
	# Mid fill crescent (second stroke slightly shorter radius)
	ctx.global_alpha(al * 0.55)
	ctx.line_width(thick * 0.55)
	ctx.begin_path()
	ctx.arc(0, 0, rad * 0.88, a0, a1)
	ctx.stroke()
	ctx.global_alpha(al)
	ctx.stroke_style("#fff")
	ctx.line_width(2.4)
	...
	ctx.rotate(swing)
	ctx.global_alpha(maxf(0.35, 1.0 - pr * 0.55))
	ctx.shadow_blur(10)
	drawMeleeWeapon(key, float(f.get("reach", 120)) * 0.92, col, charge)
	ctx.restore()
	ctx.global_alpha(1.0)
