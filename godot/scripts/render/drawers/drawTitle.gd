...
func _kb_shoot() -> String:
	# HTML kb('shoot') — default Z
	var evs := InputMap.action_get_events("shoot")
	for e in evs:
...
func drawTitle() -> void:
	...
	if img != null:
		ctx.save()
		# Draw circular portrait via texture + gold ring (clip approximated)
		...
		ctx.stroke()
		# dark ring mask outside circle is imperfect without clip — gold stroke frames it
	ctx.text_align("center")
	ctx.save()
	...
	# Mini Bobina next to outfit button
	if _bobina:
		ctx.save()
	...
	if is_touch:
		...
		st_txt = "▶  PRESS " + _kb_shoot() + "   /   TAP TO START  ◀"
