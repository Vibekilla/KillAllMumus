...
func _step(delta: float) -> void:
	...
	if dash > 0.0:
		...
		# HTML: mouse follow after real mousemove (moveT>0), or while LMB is down.
		# Touch UI: joystick only (no desktop mouse-follow).
		# Cursor is canvas-space (JoyPad.pmove / get_global_mouse_position) — viewport
		# pixels under web CSS stretch made her freeze facing last heading so shots
		# went "sideways" vs the pointer.
		...
		if JoyPad:
			mouse = JoyPad.mouse
			pointer = JoyPad.pointer
		var in_field := mouse.x > pf.position.x - 40.0 and mouse.x < pf.end.x + 40.0 \
			and mouse.y > pf.position.y - 40.0 and mouse.y < pf.end.y + 40.0
		var m_ok := JoyPad != null and float(JoyPad.mouse_move_t) > 0.0 and not is_touch and in_field
		# HTML pOK = pointer.down && !isTouch (does NOT require moveT)
		var p_ok := JoyPad != null and bool(JoyPad.pointer_down) and not is_touch
		if p_ok or m_ok:
			var src := pointer if p_ok else mouse
			...
			if edge:
				var tx := clampf(src.x + offx, pf.position.x + 8.0, pf.end.x - 8.0)
				var ty := clampf(src.y + offy, pf.position.y + 8.0, pf.end.y - 8.0)
	...
	# HTML: if(sp2>0.35) face = lerpAngle(face, atan2(vy,vx), 0.26); aim = face
	# Shots follow where she's pointed (travel heading), not a raw mouse angle.
	var sp2 := vpf.length()
	if sp2 > 0.35:
		aim = lerp_angle(aim, atan2(vpf.y, vpf.x), 0.26)
