...
const wc=r('godot/scripts/render/WorldCanvas.gd');
const main=r('godot/scenes/main/Main.tscn');
...
// HTML draw order: stage bg under entities (WorldCanvas), panel chrome on HudCanvas
ok(hc.includes('draw_panel') && !hc.includes('draw_stage_bg()'), 'HudCanvas wires panel only (no stage bg over entities)');
ok(wc.includes('draw_stage_bg') && (wc.includes('draw_boss_ambience') || wc.includes('draw_phase_veil')), 'WorldCanvas wires stage bg + playfield FX under entities');
ok(main.includes('WorldCanvas') && main.includes('HudCanvas'), 'Main has WorldCanvas + HudCanvas');
