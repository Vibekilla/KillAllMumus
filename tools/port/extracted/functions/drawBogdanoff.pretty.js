function drawBogdanoff(b, flash) {
  const R = b.r;
  // which twin is on the strings? (fake bust passes no active → reflect the live boss, else default Igor)
  const igor =
    b.active !== undefined
      ? b.active === 'igor'
      : boss && boss.twin && boss.active
        ? boss.active === 'igor'
        : true;
  const skin = flash ? '#fff' : '#c99a6a',
    skinSh = '#a2764c',
    hair = flash ? '#fff' : '#e8dcc4',
    tux = '#20202a',
    accent = igor ? '#b48ce0' : '#e0b84a';
  ...
  ctx.globalAlpha = bust ? 0.34 : 0.32 + 0.16 * Math.sin(tick * 0.1);
  ...
  ctx.strokeStyle = _hexA(accent, 0.35);
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(-R * 0.5, R * 0.2);
  ctx.lineTo(-R * 0.72, -R * 1.4);
  ctx.moveTo(R * 0.5, R * 0.2);
  ctx.lineTo(R * 0.72, -R * 1.4);
  ctx.stroke();
  ...
  ctx.quadraticCurveTo(0, -R * 0.5, -R * 0.42, -R * 0.34);
  ctx.closePath();
  ctx.fill();
  ...
  ctx.quadraticCurveTo(-R * 0.2, -R * 0.48, -R * 0.44, -R * 0.26);
  ctx.closePath();
  ctx.fill();
  // hair sweep lines
  ctx.strokeStyle = _hexA('#c9bfa2', 0.6);
  ctx.lineWidth = 1.2;
  ctx.beginPath();
  for (let i = -2; i <= 3; i++) {
    ctx.moveTo(i * R * 0.16, -R * 0.4);
  ...
  ctx.shadowBlur = 8;
  circle(-R * 0.2, 0.02 * R, R * 0.07, flash ? '#fff' : accent);
  circle(R * 0.2, 0.02 * R, R * 0.07, flash ? '#fff' : accent);
  ctx.restore();
  ctx.fillStyle = '#1a1008';
  circle(-R * 0.2, 0.02 * R, R * 0.03, '#1a1008');
  circle(R * 0.2, 0.02 * R, R * 0.03, '#1a1008');
  ...
  ctx.beginPath();
  ctx.moveTo(-R * 0.13, R * 0.38);
  ctx.quadraticCurveTo(0, R * 0.34, R * 0.13, R * 0.38);
  ctx.stroke();
  ...
  ctx.shadowColor = accent;
  ctx.shadowBlur = 8;
  ctx.fillText(igor ? 'I' : 'G', 0, -R * 0.62);
  ctx.restore();
  ctx.textAlign = 'left';
}
