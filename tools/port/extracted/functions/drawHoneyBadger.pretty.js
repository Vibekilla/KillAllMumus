function drawHoneyBadger(cx, cy, s) {
  ctx.save();
  ctx.translate(cx, cy);
  ctx.scale(s, s);
  const fur = '#2a2721',
  ...
  }; // deterministic per-strand jitter (no per-frame flicker)
  // ground shadow
  ...
  ctx.lineCap = 'round';
  ctx.beginPath();
  for (const px of [-16, -13, -10, 10, 13, 16]) {
    ctx.moveTo(px, 42);
    ctx.lineTo(px, 38);
  }
  ctx.stroke();
  ...
  ctx.ellipse(27, 3, 15, 19, -0.22, 0, 7);
  ctx.fill(); // drooping jowls
  ctx.beginPath();
  ctx.ellipse(0, 7, 31, 24, 0, 0, 7);
  ctx.fill(); // lower-face fill blends jowls
  ctx.fillStyle = cheek;
  ctx.beginPath();
  ctx.ellipse(-21, 3, 11, 12, 0, 0, 7);
  ctx.ellipse(21, 3, 11, 12, 0, 0, 7);
  ctx.fill(); // faint lighter cheeks
  ...
  for (let i = 0; i < tufts.length - 1; i++) {
    const p = tufts[i],
      nx = tufts[i + 1],
      mx = (p[0] + nx[0]) / 2,
      my = (p[1] + nx[1]) / 2;
    ctx.beginPath();
    ctx.moveTo(p[0], p[1]);
    ctx.lineTo(mx + mx * 0.12, my + my * 0.12);
    ctx.lineTo(nx[0], nx[1]);
    ctx.closePath();
    ctx.fill();
  }
  ...
  ctx.beginPath();
  for (let i = 0; i < 11; i++) {
    const a = ((-160 + i * 15) * Math.PI) / 180,
      bx = hc.x + Math.cos(a) * 30,
      by = hc.y + Math.sin(a) * 27;
    ctx.moveTo(bx, by);
  ...
  ctx.fillStyle = '#c3c7d3';
  for (const m of puffs) {
    ctx.beginPath();
    ctx.arc(m[0], m[1] + 3, m[2], 0, 7);
    ctx.fill();
  } // depth shadow under the fluff
  ...
  for (const m of puffs) {
    ...
    ctx.fill();
  } // soft top highlight on each puff → dimensional fluff
  // fine short brushed strokes for a wild, hairy texture (kept SHORT so the cap stays soft, never spiky)
  ctx.lineCap = 'round';
  for (let i = 0; i < 52; i++) {
    const jr = rnd(i, 1),
      jr2 = rnd(i, 3),
      ang = ((-180 + (i / 51) * 180) * Math.PI) / 180;
    const rr = 6 + jr * 19,
      bx = cc.x + Math.cos(ang) * rr,
      by = cc.y + Math.sin(ang) * rr * 0.6,
      ln = 2.5 + jr2 * 3;
    ctx.strokeStyle = jr < 0.32 ? 'rgba(150,156,172,0.5)' : 'rgba(255,255,255,0.72)';
    ctx.lineWidth = 0.7;
    ctx.beginPath();
    ctx.moveTo(bx, by);
    ctx.lineTo(bx + Math.cos(ang) * ln, by + Math.sin(ang) * ln - 1);
    ctx.stroke();
  }
  // just a few soft fuzzy tufts barely breaking the top edge (short + thin)
  for (let i = 0; i < 8; i++) {
    const f = i / 7,
      ang = ((-152 + f * 124) * Math.PI) / 180,
      jr = rnd(i, 7);
    const bx = cc.x + Math.cos(ang) * 16,
      by = cc.y + Math.sin(ang) * 11 - 1,
      len = 2 + jr * 4;
    ctx.strokeStyle = 'rgba(236,238,244,0.85)';
    ctx.lineWidth = 1.1;
    ctx.beginPath();
    ctx.moveTo(bx, by);
    ctx.lineTo(bx + Math.cos(ang) * len, by + Math.sin(ang) * len - 1.5);
    ctx.stroke();
  }
  ...
  ctx.fillStyle = 'rgba(180,70,80,0.38)';
  ctx.beginPath();
  ...
  ctx.fillStyle = 'rgba(150,50,60,0.5)';
  ctx.beginPath();
  ...
  ctx.fillStyle = 'rgba(210,90,110,0.4)';
  ctx.beginPath();
  ctx.ellipse(-19, -8, 6, 3.8, 0, 0, 7);
  ctx.fill();
  ctx.fillStyle = 'rgba(216,78,98,0.6)';
  ctx.beginPath();
  ctx.ellipse(19, -7, 7.5, 4.6, 0, 0, 7);
  ctx.fill();
  // little glossy pink nose, low between the eyes
  ctx.fillStyle = '#e56b86';
  ctx.beginPath();
  ctx.moveTo(-4.4, -12);
  ctx.quadraticCurveTo(0, -9.5, 4.4, -12);
  ctx.quadraticCurveTo(4.4, -7, 0, -5);
  ctx.quadraticCurveTo(-4.4, -7, -4.4, -12);
  ctx.closePath();
  ctx.fill();
  ...
  ctx.lineTo(0, -3);
  ctx.moveTo(-3, -2.4);
  ctx.quadraticCurveTo(0, -1, 3, -2.4);
  ctx.stroke();
  ctx.restore();
}
